...
public class CheckReloadDeletionsCommandTest extends AbstractBundleVersionCheckerTest {
    ...
    public void checkDeletionsTest() throws IOException {
        ...
        CLICommandInvoker.Result result = new CLICommandInvoker(rule, CheckReloadDeletionsCommand.COMMAND_NAME).asUser(admin.getId()).invoke();
        JSONObject jsonResult = JSONObject.fromObject(result.stdout());
        assertThat("Command should end correctly", result.returnCode(), is(0));
        assertThat("We should get an empty list", jsonResult.getJSONArray("deletions"), empty());
        ...
        result = new CLICommandInvoker(rule, CheckReloadDeletionsCommand.COMMAND_NAME).asUser(admin.getId()).invoke();
        jsonResult = JSONObject.fromObject(result.stdout());
        assertThat("Command should end correctly", result.returnCode(), is(0));
        assertThat("We should get a list with 2 items", jsonResult.getJSONArray("deletions"), hasSize(2));
        assertThat("Created items should be in deletions list", jsonResult.getJSONArray("deletions"), containsInAnyOrder("to-be-deleted", "to-be-deleted-too"));
